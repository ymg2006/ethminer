    ...
    for (unsigned int pIdx = 0; pIdx < platforms.size(); pIdx++)
    {
        ...
        for (auto const& device : devices)
        {
                         ...
                         platformType == ClPlatformTypeEnum::Clover))
            {
                ...
                if (clGetDeviceInfo(device.get(), CL_DEVICE_TOPOLOGY_AMD, sizeof(amd_topo),
                        &amd_topo, NULL) == CL_SUCCESS)
                {
                    std::ostringstream s;
                    s << setfill('0') << setw(2) << hex << (unsigned int)(amd_topo.bus) << ":"
                      << setw(2) << (unsigned int)(amd_topo.device) << "."
